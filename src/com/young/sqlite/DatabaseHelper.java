...
public class DatabaseHelper {
		...
		if (db != null) {
			if (db.isOpen()) {
				...
				Cursor cursor = db.rawQuery("select * from " + tableName
						+ "  where day = ? and stu_id = ? order by "
						+ SQLiteHelper.DAY_TIME + " asc", new String[] {
						day + "", id });
			...
			if (db.isOpen()) {
				...
				db.execSQL(
						"insert into " + tableName
								+ " (cur_name,teacher,place,week,day,"
	...
	public void updateSchedule(Schedule schedule) {
		if (db != null) {
			if (db.isOpen()) {
				db.execSQL(
						"update local_schedule set cur_name = ? ,teacher = ?, place = ? ,"
								+ "week = ? where _id = ?", new Object[] {
								schedule.getCurName(), schedule.getTeacher(),
								schedule.getPlace(), schedule.getWeek(),
								schedule.get_id() });
			}
		}
	...
	public void deleteSchedule(String id) {
		if (db != null) {
			if (db.isOpen()) {
				db.execSQL("delete from local_schedule where _id = ?",
						new Object[] { id });
			}
		}
	}
		...
		if (db != null) {
			if (db.isOpen()) {
				...
				do {
					...
					score.setStuId(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.STU_ID)));
					score.setGradePoint(cursor.getDouble(cursor
							.getColumnIndex(SQLiteHelper.GRADE_POINT)));
					score.setGrade(cursor.getDouble(cursor
							.getColumnIndex(SQLiteHelper.GRADE)));
					score.setCourseCredit(cursor.getDouble(cursor
							.getColumnIndex(SQLiteHelper.COURSE_CREDIT)));
					score.setCourseName(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.COURSE_NAME)));
					score.setCourseType(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.COURSE_TYPE)));
					score.setTaskNo(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.TASK_NO)));
					scores.add(score);
				} while (cursor.moveToNext());
				cursor.close();
			}
	...
	public void addScore(Score score) {
		if (db != null) {
			if (db.isOpen()) {
				db.execSQL(
						"insert into score (task_no,course_name,course_type,"
								+ "course_credit,grade,grade_point,is_show_score,stu_id)  values(?,?,?,?,?,?,?,?)",
						new Object[] { score.getTaskNo(),
								score.getCourseName(), score.getCourseType(),
								score.getCourseCredit(), score.getGrade(),
								score.getGradePoint(),
	...
	public boolean isEmpty(String tableName, String id) {
		if (db != null && db.isOpen()) {
			if (db.isOpen()) {
				Cursor cursor = db.query(tableName, null, "stu_id = " + id,
						null, null, null, null);
	...
	public void insertStudent(Student stu) {
		...
		if (db != null) {
			if (db.isOpen()) {
				db.execSQL(
						insetSql,
						new String[] { stu.getClassName(), stu.getStuName(),
								stu.getStuNum(), stu.getIDCard(), stu.getSex(),
								stu.getEthnic(), stu.getCollege(),
								stu.getMajor(), stu.getYear(),
								stu.getPoliticalStatus(), stu.getBirthDay(),
								stu.getEnterScholl(), stu.getLeftScholl() });
			}
		}
	}
	...
	public Student getStudent(String stuId) {
		...
		if (db != null) {
			if (db.isOpen()) {
				...
				while (cursor.moveToNext()) {
					stu = new Student();
					stu.setClassName(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.CLASS_NAME)));
					stu.setStuName(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.STU_NAME)));
					stu.setStuNum(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.STU_NUM)));
					stu.setIDCard(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.ID_CARD)));
					stu.setSex(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.SEX)));
					stu.setEthnic(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.ETHNIC)));
					stu.setCollege(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.COLLEGE)));
					stu.setMajor(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.MAJOR)));
					stu.setYear(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.YEAR)));
					stu.setPoliticalStatus(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.POLITICAL_STATUS)));
					stu.setBirthDay(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.BIRTH_DAY)));
					stu.setEnterScholl(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.ENTER_SCHOOL)));
					stu.setLeftScholl(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.LEAVE_SCHOOL)));
				}
			}
		}
		return stu;
	...
	public void insertScoreInfo(ScoreInfo info) {
		...
		if (db != null) {
			if (db.isOpen()) {
				db.execSQL(
						insetSql,
						new String[] { info.getStuId(),
								info.getAverageGradePoint(),
								info.getTotalGradePoint(), info.getName(),
								info.getContent() });
			}
		}
	}
	...
	public ScoreInfo getScoreInfo(String stuId) {
		...
		if (db != null) {
			if (db.isOpen()) {
				Cursor cursor = db.rawQuery(
						"select * from score_info where stu_id = ?",
						new String[] { stuId });
				if (cursor.getColumnCount() < 1) {
					return null;
				}
				while (cursor.moveToNext()) {
					info = new ScoreInfo();
					info.setAverageGradePoint(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.AVERAGE_GRADE_POINT)));
					info.setContent(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.CONTENT)));
					info.setName(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.NAME)));
					info.setTotalGradePoint(cursor.getString(cursor
							.getColumnIndex(SQLiteHelper.TOTAL_GRADE_POINT)));
					info.setStuId(stuId);
				}
			}
		}
